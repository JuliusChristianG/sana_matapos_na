...
if ($stmt) {
    // Bind parameters
    $stmt->bind_param("sssssi", $editUsername, $editFirstName, $editLastName, $editEmail, $editPosition, $patientId);
    ...
    if ($stmt->execute()) {
        ...
        $date = date("Y-m-d");
        $time = date("H:i:s");
        $logAction = "Edited patient data for user ID: $patientId";
        ...
        header("Location: adminaddpatient.php?user_id=' . $user_id"); // Redirect to a success page
    } else {
        // Error occurred
