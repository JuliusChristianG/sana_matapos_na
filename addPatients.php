...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($existingUser && $existingUser['is_verified'] == 1) {
        ...
        header("Location: emailErrorPatient.php");
        exit;}
    ...
    if (preg_match('/^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/', $password)) {
        ...
        if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            if ($mail->send()) {
            header("Location: adminAddPatient.php?registration_success=true");
            exit();
            } else {
            ...
            echo '<script>alert("Invalid email format."); window.location.href = "adminAddPatient.php";</script>';
            exit;
        }
    } else {
        // Password does not meet the requirements, display a pop-up message using JavaScript
        echo '<script>alert("Password must be 8 characters long and include at least one uppercase letter, one lowercase letter, one number, and one special character."); window.location.href = "adminAddPatient.php";</script>';
        exit;
    }
}
?>
