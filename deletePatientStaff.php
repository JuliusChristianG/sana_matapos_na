...
if (isset($_GET['user_id'])) {
    ...
    $staffUserId = $_SESSION['user_id']; // Fix the variable name here
    ...
    $date = date("Y-m-d");
    $time = date("H:i:s");
    $logAction = "Deleted a patient user (ID: $userId)";
    $logQuery = "INSERT INTO userlog (userID, uname, role, date, time, action) VALUES (?, ?, ?, ?, ?, ?)";
    $logStmt = $pdo->prepare($logQuery);
    $logStmt->bindParam(1, $staffUserId); // Fix the variable name here
